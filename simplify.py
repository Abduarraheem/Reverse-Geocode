...
def gpx_to_coordlist(gpx: "GPX",t="latlon") -> list:
    ...
    return newl
def giveCueSheet(mygpx=None,doplot=False):
    """
    ...
    linegeom = []
    lineutm = []
    if not callMapBox(coordlists[0],mapbox_key):
        return None

    for routesegment in coordlists: 
        
        j = callMapBox(routesegment,mapbox_key)
        # pprint.pprint(j)
        legs = 0
        dist_between_turns = 0
...
def main():
    ...
    mygpx = gpxpy.parse(gpx_file)
    DOPLOT = True
    giveCueSheet(mygpx, doplot=True)
